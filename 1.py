...
def train_and_predict(train_sample_feature,test_sample_feature,X_train,X_test,Y_value,svmID):

    clf=svm.SVC()
    train_sample_lable = [1 for j in range(5430)]+[0 for j in range(train_sample_feature.shape[0]-5430)]
    clf.fit(train_sample_feature,train_sample_lable)
...
# =============================================================================
    ...
    test_result =clf.decision_function(test_sample_feature)

    #print(T8-T7)
    X_train[:,svmID] = clf.decision_function(train_sample_feature)
    #X_train[:,1] = train_result[:,4]
    Y_value = train_sample_lable
    X_test[:,svmID] = test_result
    
    return(X_train,X_test,Y_value,test_result)
...
for i_nc in range(5430):
    ...
    positive_sample_index = np.argwhere(A == 1)#正样本，用数组操作
    ...
    (X_train,X_test,Y_value,test_result_DS1_FS) = train_and_predict (train_sample_DS1_FS,test_sample_DS1_FS,X_train,X_test,Y_value,0)
    T5 = time.time()
    #time1=datetime.now()
    #print(time1)
     #SVM2
# =============================================================================
    ...
    (X_train,X_test,Y_value,test_result_DS2_FS) = train_and_predict (train_sample_DS2_FS,test_sample_DS2_FS,X_train,X_test,Y_value,1)
    ...
    (X_train,X_test,Y_value,test_result_KD_FS) = train_and_predict (train_sample_KD_FS,test_sample_KD_FS,X_train,X_test,Y_value,2)
    ...
    (X_train,X_test,Y_value,test_result_DS1_KM) = train_and_predict (train_sample_DS1_KM,test_sample_DS1_KM,X_train,X_test,Y_value,3)
    ...
    (X_train,X_test,Y_value,test_result_DS2_KM) = train_and_predict (train_sample_DS2_KM,test_sample_DS2_KM,X_train,X_test,Y_value,4)
    ...
    (X_train,X_test,Y_value,test_result_KD_KM) = train_and_predict (train_sample_KD_KM,test_sample_KD_KM,X_train,X_test,Y_value,5)
    ...
    localrank_pos.append(predict_0_localrank[i_local-1]) #找出 找出A中1变0对应实例的locai排名


    A[ConnectDate[i_nc,1], ConnectDate[i_nc,0]] = 1  # 将A中变0元素变回1 
    
    globalrank_posTemp = np.array(globalrank_pos)
    np.savetxt('result_%d.dat'%(i_nc+1),globalrank_posTemp)
    localrank_posTemp = np.array(localrank_pos)
    np.savetxt('localResult_%d.dat'%(i_nc+1),localrank_posTemp)
